          ...
          <Col xs={6}>
            <Row>
              <Col xs={2} collapseRight>
                <Checkbox defaultChecked={ dchecked } name={ self.props['answerTo'] + '-choice-' + (index+1) + '-correct'}/></Col>
              <Col xs={10} collapseLeft>
                <Textarea rows='2' name={ self.props['answerTo'] + '-choice-' + (index+1)  } placeholder='New choice...'>{choice.content}</Textarea>
              </Col>
            </Row>
          </Col>  
          <Col xs={6} collapseLeft>
              <ButtonGroup>
                <a onClick={ self.addChoice }><Icon className={'fg-orange'} style={{fontSize: 28}} glyph='icon-fontello-plus' /></a>
                <a style={{ paddingLeft: 10 }}onClick={ self.removeChoice.bind(this, 'choice-' + (index+1) ) }><Icon className={'fg-orange'} style={{fontSize: 28}} glyph='icon-fontello-minus' /></a>
              </ButtonGroup>
          </Col>
        </Row>);    
